...
import { Button, Input, Badge } from 'src/core/primitives';
...
export function AudienceForm({ initialCompany = null, onSubmit, submitting = false }) {
  ...
  const activeFilters = useMemo(() => {
    const n =
      location.length + industry.length + company.length + pastCompany.length +
      school.length + networkDistance.length +
      (keywords.trim() ? 1 : 0) + (title.trim() ? 1 : 0);
    return n;
  }, [location, industry, company, pastCompany, school, networkDistance, keywords, title]);
  ...
  const submit = (e) => {
    ...
    onSubmit(
      usingUrl
        ? { searchUrl: url.trim(), name: name.trim() }
        : { filters: buildFilters(), name: name.trim() },
    );
  };
    ...
    <form onSubmit={submit} className="flex flex-col">
      <div className="p-[var(--ui-pad-lg)] flex flex-col gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          />
          {usingUrl && (
            <Button type="button" variant="ghost" onClick={() => setUrl('')} disabled={submitting}>
              Clear URL
            </Button>
          )}
        </div>
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          <FilterTagPicker type="LOCATION" label="Location" placeholder="Search a city or region…"
            value={location} onChange={setLocation} disabled={filtersOff} />
          <FilterTagPicker type="INDUSTRY" label="Industry" placeholder="Search an industry…"
            value={industry} onChange={setIndustry} disabled={filtersOff} />
          <FilterTagPicker type="COMPANY" label="Current company" placeholder="Search a company…"
            value={company} onChange={setCompany} disabled={filtersOff} />
          <FilterTagPicker type="COMPANY" label="Past company" placeholder="Search a company…"
            value={pastCompany} onChange={setPastCompany} disabled={filtersOff} />
          <FilterTagPicker type="SCHOOL" label="School" placeholder="Search a school…"
          ...
          <div className="flex flex-col gap-2">
            ...
            <Input id="audience-keywords" fullWidth value={keywords} disabled={filtersOff}
              onChange={(e) => setKeywords(e.target.value)} placeholder="e.g. co-founder" />
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <Input id="audience-title" fullWidth value={title} disabled={filtersOff}
              onChange={(e) => setTitle(e.target.value)} placeholder="e.g. VP Sales" />
          </div>
        </div>
      ...
      <div className="flex items-center gap-3 p-[var(--ui-pad-lg)] border-t border-[var(--ui-border-hairline)] bg-[var(--ui-surface-sunken)]">
        ...
        {!usingUrl && activeFilters > 0 && (
          <Badge tone="accent">{activeFilters} {activeFilters === 1 ? 'filter' : 'filters'}</Badge>
        )}
