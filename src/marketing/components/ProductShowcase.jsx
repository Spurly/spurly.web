...
export default function ProductShowcase() {
  ...
  const next = () => {
    iRef.current = (iRef.current + 1) % ORDER.length;
    setActive(ORDER[iRef.current]);
  };
  const start = () => {
    stop();
  ...
  const onTab = (key) => {
    iRef.current = ORDER.indexOf(key);
    setActive(key);
    start();
  };
        ...
        <div className="shot-stage reveal d1" ref={rootRef}>
          <div className="browser-wrap">
            ...
            <div className="shot-float fl-1 reveal d2">
              ...
              <div>
                <small>Personalized &amp; sent</small>
                <strong className="tnum">9 / 9 leads</strong>
              </div>
            </div>
            <div className="shot-float fl-2 reveal d3">
              ...
              <div>
                ...
                <strong className="tnum">982.0</strong>
              </div>
            </div>
          </div>
          <div
            className="shot-tabs"
            role="tablist"
            aria-label="Spurly workflow steps"
          >
            {ORDER.map((key, idx) => (
              <button
                key={key}
                ...
                onClick={() => onTab(key)}
              >
                <span className="st-i">{idx + 1}</span>
                {LABELS[key]}
              </button>
            ))}
          </div>
