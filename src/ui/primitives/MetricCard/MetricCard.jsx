export function MetricCard({ label, value, change, delta, changeType, icon, variant = 'solid', hint, hintColor }) {
  ...
  return (
    <div className={`relative rounded-[var(--ui-radius-lg)] p-[var(--ui-pad-lg)] overflow-hidden ${cardBase}`}>
      ...
      >{label}</p>
      <p className="text-[24px] font-medium tracking-[-0.012em] text-[var(--ui-text-primary)] leading-none tabular-nums">{value}</p>
      {deltaValue !== undefined && (
        <div className="flex items-center gap-1.5 mt-3">
          ...
          <span className="text-[12px] text-[var(--ui-text-tertiary)]">vs last 7 days</span>
        </div>
      )}
