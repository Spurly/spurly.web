  ...
  return [
    {
      ...
      render: (value, row) => (
        <span className="flex items-center gap-2 min-w-0">
          ...
          {(row.rank ?? 0) > 0 && (
            <span className="shrink-0 text-[length:var(--ui-t-meta)] text-[var(--ui-text-tertiary)]">T{row.rank}</span>
          )}
        </span>
      ),
    },
    {
      ...
      width: 160,
      render: (value) => (
        <code className="truncate font-mono text-[length:var(--ui-t-meta)] px-1.5 py-0.5 rounded-[var(--ui-radius-xs)] bg-[var(--ui-surface-sunken)] text-[var(--ui-text-secondary)]">
          {value}
        </code>
      ),
    },
