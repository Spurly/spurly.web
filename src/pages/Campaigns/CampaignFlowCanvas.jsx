 ...
 *   - sent     → solid purple
 *   - accepted → solid green (with a check)
 *   - failed   → red
 *   - skipped  → muted
...
const DONE_STATUSES = new Set(['sent', 'accepted', 'failed', 'skipped']);
...
const STATUS_META = {
  ...
  accepted: { label: 'Accepted', color: 'var(--green)',         tint: 'var(--green-tint)',     Icon: Check },
...
export function CampaignFlowCanvas({ members = [], actionType, sending = false, status }) {
  ...
  const counts = useMemo(() => {
    const c = { pending: 0, sent: 0, accepted: 0, failed: 0, skipped: 0 };
    members.forEach((m) => {
  ...
  return (
    <div className="campaign-canvas relative h-full w-full overflow-y-auto">
      ...
      <div className="sticky top-0 z-20 flex flex-wrap items-center gap-2 px-6 py-3 backdrop-blur-md"
        ...
        <Stat label="Accepted" value={counts.accepted} icon={Check} color="var(--green)" />
