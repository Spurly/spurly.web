...
function NetworkReachCard({ connectionDegrees, donutSize = 140 }) {
  ...
  const segments = [
    ...
    ...(unknown > 0 ? [{ value: unknown, color: '#d1d5db', label: 'Unknown', count: unknown }] : []),
  ];
  ...
  return (
    <div className="bg-[var(--surface-card)] border border-[var(--border-hairline)] shadow-[var(--shadow-sm)] rounded-[18px] p-5">
      <div className="flex items-center gap-2.5 mb-4">
        ...
        <div>
          <p className="text-[14px] font-semibold" style={{ color: '#06b6d4' }}>Network reach</p>
          <p className="text-[12px] text-[var(--text-tertiary)]">Connection degree mix</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4">
        <div className="relative shrink-0">
          <DonutChart
            segments={total > 0 ? segments : [{ value: 1, color: 'var(--surface-sunken)' }]}
            total={total || 1}
            size={donutSize}
            strokeWidth={strokeWidth}
          />
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[12px] text-[var(--text-secondary)] mb-2.5">2nd° — warmest path</p>
          {segments.map((seg) => (
            <div key={seg.label} className="flex items-center justify-between mb-1.5 last:mb-0">
              <div className="flex items-center gap-1.5">
                <span
                  className="w-2 h-2 rounded-full shrink-0"
                  style={{ background: seg.color }}
                />
                <span className="text-[11.5px] text-[var(--text-secondary)]">{seg.label}</span>
              </div>
...
function EngageReadyCard({ connectionDegrees, total }) {
  ...
  const connect = connectionDegrees.second + connectionDegrees.third + connectionDegrees.unknown;
  const max = Math.max(total || 0, reachOut + connect, 1);

  const bars = [
    { label: 'Total leads',     value: total,    pct: 100,                    color: '#06b6d4' },
    { label: 'Reach out ready', value: reachOut, pct: (reachOut / max) * 100, color: '#7c3aed' },
    { label: 'Connect ready',   value: connect,  pct: (connect / max) * 100,  color: '#f97316' },
  ];
  ...
  return (
    <div className="relative rounded-[18px] p-6 overflow-hidden bg-[var(--surface-card)] border border-[var(--border-hairline)] shadow-[var(--shadow-sm)]">
      ...
      <p className="text-[12px] font-semibold uppercase tracking-[0.04em] mb-4" style={{ color: '#7c3aed' }}>
        Engage-Ready
      </p>
        ...
        {bars.map((bar) => (
          <div key={bar.label}>
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-[11.5px] text-[var(--text-secondary)]">{bar.label}</span>
              <span
    ...
    <DashboardLayout>
      <div className="relative flex flex-col h-full overflow-hidden">
        ...
        <div className="glass-chrome border-b border-[var(--separator)] px-6 py-4 shrink-0 flex items-center justify-between gap-4">
          <div>
            <h1 className="text-[20px] font-bold tracking-[-0.018em] text-[var(--text-primary)]">Signals</h1>
            <p className="text-[13px] text-[var(--text-secondary)] mt-0.5">
              Outreach intelligence across {pagination.total.toLocaleString()} captured leads
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1.5 h-9 px-3 rounded-[10px] border border-[var(--border-hairline)] bg-[var(--surface-card)]">
              ...
              <span className="text-[13px] font-medium text-[var(--text-secondary)]">
                {sessions.active > 0 ? `${sessions.active} sessions` : 'Sessions'}
              </span>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          {/* Top 4-card row */}
          <div className="grid gap-4" style={{ gridTemplateColumns: '1.5fr 1.5fr 1fr 1fr' }}>
            <EngageReadyCard
          ...
          <div className="bg-[var(--surface-card)] border border-[var(--border-hairline)] shadow-[var(--shadow-sm)] rounded-[18px] overflow-hidden flex flex-col">
            ...
            <div className="px-5 pt-5 pb-4 flex items-start justify-between gap-4 shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[14px] font-semibold text-[var(--text-primary)]">Priority queue</p>
